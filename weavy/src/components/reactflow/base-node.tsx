...
export function BaseNode({ className, status = "initial", ...props }: BaseNodeProps) {
  return (
    <div
      className={cn(
    ...
    >
      ...
      {status === "loading" && (
        <Loader2Icon className="absolute -right-0.5 -bottom-0.5 size-2 text-purple-700 stroke-3 animate-spin"/>
      )}
      </div>
  );
}
